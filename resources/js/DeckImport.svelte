<script>
    ...
    let importProgressPercentage = $state(0);
    let fileName = $state('');
    let fileInputElement = $state(null);

    run(() => {
        importProgressPercentage = round((importedItems / totalItems) * 100);
    });
        ...
        <div class="d-grid">
            ...
            {#if importInProgress}
                <div class="progress" role="progressbar" aria-valuenow="{importProgressPercentage}" aria-valuemin="0" aria-valuemax="100">
                    <div class="progress-bar" style="width: {importProgressPercentage}%">{importProgressPercentage}%</div>
                </div>
            {/if}
        </div>
    </div>
